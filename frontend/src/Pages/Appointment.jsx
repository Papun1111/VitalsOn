...
const Appointment = () => {
  ...
  const getAvailableSlot = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const slotCount = isWeekend ? 4 : 8; // Fewer slots on weekends
            ...
            <div className="space-y-2">
              {docSlots.length !== 0 && docSlots.map((item, index) => (
                ...
                >
                  ...
                  <p className="text-gray-600">{item[0] && item[0].datetime.getDate()}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="mt-4">
              {/* Display only the time slots for the selected date */}
